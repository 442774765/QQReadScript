...
* 在 `jobs` 下面找到 `qq_read` 任务，前面几个参数就是该任务的基本设置，自己看注释。在最后一个 `ACCOUNT` 参数下面填写你的账号信息，将先前抓到的获取书籍 `url` 和 `headers` 以及 `body` 对应填入配置文件里面的 `BOOK_URL`，`HEADERS` 和 `BODY` （注意冒号后面的空格，不要带引号！）
